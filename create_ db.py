    ...
    if not databaseexisted:  # First time creating teh database. Create teh tables
        ...
        for line in file:
            ...
            if line.startswith("C"):
                id = words[1].strip(' ')
                course_name = words[2].strip(' ')
                student = words[3].strip(' ')
                number_of_students = words[4].strip(' ')
                class_id = words[5].strip(' ')
                #endline = words[6].split('\n')
                course_length = words[6][1:].strip(' ')
                dbcon.execute(
